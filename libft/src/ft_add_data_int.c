...
/*   Updated: 2016/11/07 19:45:54 by pbourdon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_dlist		*dlist_prepend2(t_dlist *p_list, int data)
{
	t_node		*p_new;
...
t_dlist		*dlist_append2(t_dlist *p_list, int data)
{
	t_node		*p_new;
...
{
	if (p_list != NULL)
		p_list = dlist_append2(p_list, data);
	p_list->pos = p_list->pos + 1;
	p_list->length = p_list->length + 1;
	return (p_list);
}

t_dlist		*ft_add_data_int2(t_dlist *p_list, int data)
{
	if (p_list != NULL)
		p_list = dlist_append2(p_list, data);
